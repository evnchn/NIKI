                    ...
                    master_message_list.append(
                        {
                            ...
                            'content': json.dumps(
                                {'success': True, 'text': text, 'emotion': emotion}
                            ),
                        }
                    )
...
async def handle_user_input(message: str):
    if my_shared_state.pending_tool_call_id:
        random_nonce = json.loads(my_shared_state.pending_tool_args).get(
            'random_nonce', ''
        )
        if my_shared_state.pending_tool_name == 'wait_for_user_engagement':
            content = json.dumps(
                {'engagement': message.lower() == 'yes', 'random_nonce': random_nonce}
            )
        elif my_shared_state.pending_tool_name == 'wait_for_user_choose_photo':
            content = json.dumps(
                {'chosen_photo': message, 'random_nonce': random_nonce}
            )
        elif my_shared_state.pending_tool_name == 'detect_presence':
            content = json.dumps(
                {'presence_detected': message == 'yes', 'random_nonce': random_nonce}
            )
        elif my_shared_state.pending_tool_name == 'capture_photos':
            content = json.dumps(
                {'capture_success': message == 'yes', 'random_nonce': random_nonce}
            )
        elif my_shared_state.pending_tool_name == 'print_photo':
            content = json.dumps(
                {'print_success': message == 'yes', 'random_nonce': random_nonce}
            )
        else:
            content = json.dumps(
                {
    ...
    def refresh():
        ...
        with main_container:
            if mode != 'niki':
                        ...
                        if 'tool_calls' in msg:
                            for tool_call in msg['tool_calls']:
                                if (
                                    ...
                                    ui.label(
                                        f"AI is calling {tool_call['function']['name']}..."
                                    )
                    elif role == 'tool':
                        result = json.loads(content)
                        if 'framing_quality' in result:
                            ui.label(
                                f"Camera framing assessed: {result['framing_quality']} - {result['details']}"
                            )
                            ...
                            ui.label(
                                f"Presence detected: {result['presence_detected']}"
                            )
                        elif 'capture_success' in result:
                            ui.label(
                                f"Photo capture success: {result['capture_success']}"
                            )
                ...
                if (
                    last_tool_result
                    and 'print_success' in last_tool_result
                    and last_tool_result['print_success']
                ):
                    ...
                    with ui.row().classes('w-full'):
                        for i, photo in enumerate(photo_list):
                            img = ui.image(
                                f'/user_photos/{os.path.basename(photo)}'
                            ).classes('w-1/3')
            ...
            if (
                my_shared_state.turn == 'user'
                and my_shared_state.pending_tool_name == 'wait_for_user_engagement'
            ):
                ...
                my_shared_state.turn == 'user'
                and my_shared_state.pending_tool_name == 'wait_for_user_choose_photo'
            ):
                with ui.row():
                    for i, photo in enumerate(photo_list):
                        img = ui.image(
                ...
                if mode == 'admin':
                    ...
                    if my_shared_state.pending_tool_name == 'capture_photos':
                        ...
                        my_button(
                            'Say cheese',
                            on_click=lambda: play_tts(
                                'Say cheese! Smile big!', 'HAPPY'
                            ),
                        )
                        my_button('3 2 1', on_click=lambda: play_tts('3 2 1', 'HAPPY'))
                else:
                    ui.label(
                        f'Waiting for admin to handle {my_shared_state.pending_tool_name}...'
                    )
